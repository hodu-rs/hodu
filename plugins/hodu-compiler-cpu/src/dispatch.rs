//! Dispatch manifest for CPU kernel execution
//!
...
pub struct KernelDispatch {
    /// Kernel function name (e.g., "hodu_cpu_add_f32")
    pub kernel_name: String,
    /// Op type for C codegen
    pub op_type: String,
    /// Input buffer IDs
...
impl DispatchManifest {
    ...
    pub fn from_snapshot(snapshot: &Snapshot) -> Self {
        ...
        let mut tensor_to_buffer: std::collections::HashMap<usize, usize> =
            std::collections::HashMap::new();
        // Map tensor IDs to their dtypes
        let mut tensor_to_dtype: std::collections::HashMap<usize, DType> =
            std::collections::HashMap::new();
        ...
        let inputs: Vec<TensorSpec> = snapshot
            ...
            .map(|input| {
                ...
                tensor_to_dtype.insert(input.id.0, input.dtype);
                num_buffers += 1;
                TensorSpec {
                    ...
                    size_bytes: input.shape.size() * input.dtype.get_size_in_bytes(),
                }
            })
            .collect();
        ...
        let constants: Vec<ConstantData> = snapshot
            ...
            .map(|constant| {
                ...
                tensor_to_dtype.insert(constant.id.0, constant.dtype);
                num_buffers += 1;
                ConstantData {
                    buffer_id,
                    shape: constant.shape.dims().to_vec(),
                    dtype: dtype_to_string(constant.dtype),
                    size_bytes: constant.shape.size() * constant.dtype.get_size_in_bytes(),
        ...
        for node in &snapshot.nodes {
            ...
            // For Shape ops and Dummy, reuse input buffer (no actual computation)
            let is_noop = matches!(node.op, Op::Shape(_) | Op::ShapeScalars(_) | Op::Dummy);
            let output_buffer = if is_noop && !input_buffers.is_empty() {
                // Reuse input buffer for shape ops
                let input_buf = input_buffers[0];
                tensor_to_buffer.insert(node.output_id.0, input_buf);
                tensor_to_dtype.insert(node.output_id.0, node.output_dtype);
                input_buf
            } else {
                let buf = num_buffers;
                tensor_to_buffer.insert(node.output_id.0, buf);
                tensor_to_dtype.insert(node.output_id.0, node.output_dtype);
                num_buffers += 1;
                buf
            };

            // Get input dtype for cast operations
            let input_dtype = node
                .input_ids
                .first()
                .and_then(|id| tensor_to_dtype.get(&id.0).copied());

            let (kernel_name, op_type) = op_to_kernel_info(&node.op, node.output_dtype, input_dtype);
            let output_size = node.output_layout.size();
            ...
            let scalars = extract_scalars_from_params(node.params.as_ref());
            ...
            dispatches.push(KernelDispatch {
                ...
                output_buffer,
                output_size,
                output_dtype: dtype_to_string(node.output_dtype),
                metadata,
                scalars,
            });
        }

        // Build a map of tensor_id -> (shape, dtype, size_bytes)
        let mut tensor_info: std::collections::HashMap<usize, (Vec<usize>, DType, usize)> =
            std::collections::HashMap::new();
        for node in &snapshot.nodes {
            tensor_info.insert(
                node.output_id.0,
                (
                    ...
                    node.output_dtype,
                    node.output_layout.size() * node.output_dtype.get_size_in_bytes(),
                ),
            );
        }
        ...
        let outputs: Vec<TensorSpec> = snapshot
            ...
            .map(|target| {
                ...
                let (shape, dtype, size_bytes) = tensor_info
                    .get(&target.id.0)
                    .cloned()
                    .unwrap_or_else(|| (Vec::new(), DType::F32, 0));
                TensorSpec {
...
fn op_to_kernel_info(op: &Op, output_dtype: DType, input_dtype: Option<DType>) -> (String, String) {
    let dtype_suffix = dtype_to_string(output_dtype);

    match op {
        Op::Binary(binary_op) => (
            format!("hodu_cpu_{}_{}", binary_op, dtype_suffix),
            "binary".to_string(),
        ),
        Op::BinaryLogical(op) => (
            format!("hodu_cpu_{}_{}", op, dtype_suffix),
            "binary_logical".to_string(),
        ),
        Op::Cmp(op) => (
            format!("hodu_cpu_{}_{}", op, dtype_suffix),
            "cmp".to_string(),
        ...
        Op::UnaryLogical(op) => (
            format!("hodu_cpu_{}_{}", op, dtype_suffix),
            "unary_logical".to_string(),
        ),
        Op::UnaryScalar(op) => (
            format!("hodu_cpu_{}_{}", op, dtype_suffix),
            "unary_scalar".to_string(),
        ),
        Op::Matrix(op) => (
            format!("hodu_cpu_{}_{}", op, dtype_suffix),
            "matrix".to_string(),
        ...
        Op::Concat(_) => (
            format!("hodu_cpu_concat_{}", dtype_suffix),
            "concat".to_string(),
        ),
        Op::Split(_) => (
            format!("hodu_cpu_split_{}", dtype_suffix),
            "split".to_string(),
        ),
        Op::Indexing(op) => (
            format!("hodu_cpu_{}_{}", op, dtype_suffix),
        ...
        Op::Shape(_) | Op::ShapeScalars(_) => ("noop".to_string(), "shape".to_string()),
        Op::Cast(_) => {
            // Cast kernel: hodu_cpu_cast_{input_dtype}_to_{output_dtype}
            let input_suffix = input_dtype.map(dtype_to_string).unwrap_or_else(|| dtype_suffix.clone());
            (
                format!("hodu_cpu_cast_{}_to_{}", input_suffix, dtype_suffix),
                "cast".to_string(),
            )
        }
        Op::Memory(op) => (
        ...
        Op::Dummy => ("noop".to_string(), "dummy".to_string()),
    }
}

fn extract_scalars_from_params(params: Option<&OpParams>) -> Vec<f64> {
    use hodu_core::op_params;
    match params {
        Some(OpParams::UnaryScalar(op_params::UnaryScalarParams { scalar })) => {
            vec![scalar.to_f64()]
        }
        Some(OpParams::CmpScalar(op_params::CmpScalarParams { scalar })) => {
            vec![scalar.to_f64()]
        }
        _ => vec![],
    }
...
fn build_metadata(
    ...
    match op {
        // Binary ops
        Op::Binary(_) | Op::BinaryLogical(_) | Op::Cmp(_) => {
        ...
        Op::Windowing(_) => {
            ...
            {
                if !input_layouts.is_empty() {
                    let flat_padding: Vec<usize> =
                        padding.iter().flat_map(|(lo, hi)| vec![*lo, *hi]).collect();
        ...
        Op::Cast(_) => {
            if !input_layouts.is_empty() {
                op_metadatas::cast_metadata(&input_layouts[0])
            } else {
                op_metadatas::cast_metadata(output_layout)
            }
        }
        ...
        Op::Memory(_) => {
            if !input_layouts.is_empty() {
                op_metadatas::contiguous_metadata(&input_layouts[0])
            } else {
                op_metadatas::contiguous_metadata(output_layout)
            }
        }
