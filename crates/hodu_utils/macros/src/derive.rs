...
pub fn derive_dataset_impl(input: TokenStream) -> TokenStream {
    ...
    let expanded = quote! {
        impl #impl_generics #hodu_utils_path::data::Dataset for #name #ty_generics #where_clause {
            fn len(&self) -> usize {
                self.len()
            }

            fn get(&self, index: usize) -> #hodu_core_path::error::HoduResult<#hodu_utils_path::data::DataItem> {
                self.get(index)
            }
        }
    };
