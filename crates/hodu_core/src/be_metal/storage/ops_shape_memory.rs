...
pub fn call_ops_flip(input_storage: &MetalStorage, input_layout: &Layout, dims: &[usize]) -> HoduResult<MetalStorage> {
    ...
    let output_buffer = device.new_buffer(num_els, dtype, "flip_output")?;
    let metadata = crate::op_metadatas::flip_metadata(input_layout, dims);

    let kernel_name = format!("hodu_metal_flip_{}", dtype);
