...
pub fn call_ops_trace(storage: &CudaStorage, layout: &Layout) -> HoduResult<CudaStorage> {
    let shape = layout.shape();
    let ndim = shape.ndim();
    ...
    let kernel_name = format!("hodu_cuda_trace_{}", dtype);
    let kernel_name_static = crate::cache::kernel::get_kernel_name(kernel_name);
    let kernel = kernels::Kernel(kernel_name_static);
    ...
    match &storage.data {
        CudaStorageData::F32(input) => Ok(CudaStorage::new(
            device_id,
            Arc::clone(&device_arc),
            CudaStorageData::F32(call_trace!(input, f32)),
        )),
        #[cfg(feature = "f64")]
        CudaStorageData::F64(input) => Ok(CudaStorage::new(
            device_id,
            Arc::clone(&device_arc),
            CudaStorageData::F64(call_trace!(input, f64)),
        )),
        CudaStorageData::F16(input) => Ok(CudaStorage::new(
            device_id,
            Arc::clone(&device_arc),
            CudaStorageData::F16(call_trace!(input, half::f16)),
        )),
        CudaStorageData::BF16(input) => Ok(CudaStorage::new(
            device_id,
            Arc::clone(&device_arc),
            CudaStorageData::BF16(call_trace!(input, half::bf16)),
        )),
        _ => Err(HoduError::BackendError(format!(
            "trace not supported for dtype {}",
            storage.dtype()
        ))),
    }
}
