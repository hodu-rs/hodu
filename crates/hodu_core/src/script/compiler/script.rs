...
impl ScriptCompiler {
    pub fn new(compiler_type: Compiler, runtime: Runtime, device: Device) -> Self {
        Self {
            compiler_type,
            runtime,
            device,
...
impl CompilerT for ScriptCompiler {
    ...
    fn compile(&self, module: &Module, options: CompileOptions) -> HoduResult<CompiledModule> {
        ...
        // Pre-convert constants for HODU backend, leave empty for XLA
        let constant_storages = match self.runtime {
            Runtime::HODU => self.convert_constants_to_device(&constant_data, options.device)?,
            #[cfg(feature = "xla")]
            Runtime::XLA => HashMap::new(), // XLA builds constants during execution
        };
