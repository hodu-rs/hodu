...
__device__ inline size_t reflect_index(long idx, size_t size) {
    if (idx < 0) {
        idx = -idx;
    }
    if ((size_t)idx >= size) {
...
__device__ inline size_t replicate_index(long idx, size_t size) {
    if (idx < 0)
        return 0;
    if ((size_t)idx >= size)
        return size - 1;
...
__device__ inline size_t circular_index(long idx, size_t size) {
        ...
        for (unsigned int i = blockIdx.x * blockDim.x + threadIdx.x; i < num_els;                  \
            ...
            for (size_t d = num_dims; d-- > 0;) {                                                  \
                ...
                input_idx += (size_t)in_coord * in_stride;                                         \
                                                                                                   ...
                                                                                                   \
            output[i] = in_bounds ? input[input_idx] : pv;                                         \
        }                                                                                          \
    }
...
PAD_CONSTANT_OP(__nv_fp8_e4m3, pad_constant_f8e4m3)
PAD_CONSTANT_OP(__nv_fp8_e5m2, pad_constant_f8e5m2)
PAD_CONSTANT_OP(__nv_bfloat16, pad_constant_bf16)
PAD_CONSTANT_OP(__half, pad_constant_f16)
PAD_CONSTANT_OP(float, pad_constant_f32)
PAD_CONSTANT_OP(double, pad_constant_f64)
...
PAD_REFLECT_OP(__nv_fp8_e4m3, pad_reflect_f8e4m3)
PAD_REFLECT_OP(__nv_fp8_e5m2, pad_reflect_f8e5m2)
PAD_REFLECT_OP(__nv_bfloat16, pad_reflect_bf16)
PAD_REFLECT_OP(__half, pad_reflect_f16)
PAD_REFLECT_OP(float, pad_reflect_f32)
PAD_REFLECT_OP(double, pad_reflect_f64)
...
PAD_REPLICATE_OP(__nv_fp8_e4m3, pad_replicate_f8e4m3)
PAD_REPLICATE_OP(__nv_fp8_e5m2, pad_replicate_f8e5m2)
PAD_REPLICATE_OP(__nv_bfloat16, pad_replicate_bf16)
PAD_REPLICATE_OP(__half, pad_replicate_f16)
PAD_REPLICATE_OP(float, pad_replicate_f32)
PAD_REPLICATE_OP(double, pad_replicate_f64)
...
PAD_CIRCULAR_OP(__nv_fp8_e4m3, pad_circular_f8e4m3)
PAD_CIRCULAR_OP(__nv_fp8_e5m2, pad_circular_f8e5m2)
PAD_CIRCULAR_OP(__nv_bfloat16, pad_circular_bf16)
PAD_CIRCULAR_OP(__half, pad_circular_f16)
PAD_CIRCULAR_OP(float, pad_circular_f32)
PAD_CIRCULAR_OP(double, pad_circular_f64)
