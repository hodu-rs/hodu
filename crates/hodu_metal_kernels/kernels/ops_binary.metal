...
BINARY_OP(bfloat, bfloat, rem_bf16,
          static_cast<bfloat>(fmod(static_cast<float>(x), static_cast<float>(y))));
...
BINARY_OP(half, half, rem_f16, fmod(x, y));
...
BINARY_OP(float, float, rem_f32, fmod(x, y));
...
BINARY_OP(uint8_t, uint8_t, rem_u8, x % y);
...
BINARY_OP(uint16_t, uint16_t, rem_u16, x % y);
...
BINARY_OP(uint32_t, uint32_t, rem_u32, x % y);
...
BINARY_OP(uint64_t, uint64_t, rem_u64, x % y);
...
BINARY_OP(int8_t, int8_t, rem_i8, x % y);
...
BINARY_OP(int16_t, int16_t, rem_i16, x % y);
...
BINARY_OP(int32_t, int32_t, rem_i32, x % y);
...
BINARY_OP(int64_t, int64_t, rem_i64, x % y);
