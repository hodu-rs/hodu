...
///
/// # Arguments
/// * `kernel` - Binary operation kernel to execute (add, sub, mul, div, etc.)
...
///     0,      // rhs_offset
/// ];
/// call_ops_binary(&device, &command_buffer, &kernels, add::F32,
...
pub fn call_ops_binary(
    ...
    let pipeline = kernels.load_pipeline(device, Source::Binary, kernel.0)?;
