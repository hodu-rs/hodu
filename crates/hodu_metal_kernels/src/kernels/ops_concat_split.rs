...
/// ];
/// call_ops_concat(&device, &command_buffer, &kernels, concat::F32,
...
pub fn call_ops_concat(
    ...
    let pipeline = kernels.load_pipeline(device, Source::ConcatSplit, kernel.0)?;
...
///     0,      // split_offset
/// ];
/// call_ops_split(&device, &command_buffer, &kernels, split::F32,
///            input_buffer, &output, &metadata)?;
/// ```
