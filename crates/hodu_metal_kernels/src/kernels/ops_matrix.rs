...
///
/// # Arguments
/// * `kernel` - Matmul kernel (e.g., matmul::F32)
...
///     2, 3, 2 // M, K, N
/// ];
/// call_ops_matmul(&device, &command_buffer, &kernels, matmul::F32,
...
pub fn call_ops_matmul(
    ...
    let pipeline = kernels.load_pipeline(device, Source::Matrix, kernel.0)?;
...
/// or batched operations, consider using `call_ops_matmul` instead.
///
/// # Example
/// ```ignore
...
///     0,  // rhs_offset
/// ];
/// call_ops_dot(&device, &command_buffer, &kernels, dot::F32,
