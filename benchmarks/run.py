...
def main():
    ...
    if len(sys.argv) < 2:
        print(
            "Usage: python run.py --bench=mlp|cnn-training [--cpu] [--metal] [--cuda] [--xla] [--quiet] [-w=N] [-i=N]"
        )
    ...
    if not bench_type or bench_type not in ["mlp", "cnn-training"]:
        print("Error: Must specify --bench=mlp, or --bench=cnn-training")
        sys.exit(1)
